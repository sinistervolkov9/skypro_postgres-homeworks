...
-- 1. "имя контакта" и "город" (contact_name, city)
-- из таблицы customers (только эти две колонки)
select contact_name, city from customers;

-- 2. идентификатор заказа
-- и разницу между датами формирования (order_date) заказа
-- и его отгрузкой (shipped_date) из таблицы orders
select order_id, (shipped_date - order_date) as difference_in_days
from orders;

-- 3. все города без повторов,
-- в которых зарегистрированы заказчики (customers)
